...
public class MainActivity extends PluginActivity {
    ...
    public void processImage(String thetaPicturePathL, String thetaPicturePathR) {
        ...
        try {
            saveBmp(bmp, getDepthImagePath(thetaPicturePathL));
            MyLog.Log("processImage() depth-map saved as picture at : "+getDepthImagePath(thetaPicturePathL));
        } catch (Throwable e) {
    ...
    // generate the name of the new depth-map file
    ...
    // the latest modified two pictures in time.
    private String[] getLatestTwoPict() {
        String path = Environment.getExternalStorageDirectory().toString()+"/DCIM/100RICOH/";
        MyLog.Log("Path: " + path);
        File directory = new File(path);
        File[] files = directory.listFiles();
        MyLog.Log("Number of pictures on camera: "+ files.length);
        long newest = 0, newestNext = 0;
        String[] foundNames = {null,null};
        for (File f: files) {
            if (! f.getName().contains("_depth")) {
                if (f.lastModified() > newest) {
                    foundNames[1] = foundNames[0];
                    foundNames[0] = path + f.getName();
                    newestNext = newest;
                    newest = f.lastModified();
                } else if (f.lastModified() > newestNext) {
                    foundNames[1] = path + f.getName();
                    newestNext = f.lastModified();
                }
            }
        }
        int foundCnt = ((foundNames[0]!=null)?1:0) + ((foundNames[1]!=null)?1:0);
        if (foundCnt == 2) {
            return foundNames;
        } else {
            MyLog.Log("getLatestTwoPict only got " + foundCnt + " pictures, need two !");
